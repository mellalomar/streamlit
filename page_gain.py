...
# Compatibility helper for query params (similar to app.py)
...
def show_page():
    """Affiche la page des gains par propriétaire.

    Cette fonction ne ré-appelle pas `st.set_page_config` pour éviter les erreurs
    si l'application principale l'a déjà appelé.
    """

    st.title("Gains par PROPRIETAIRE — Extraction 2025")

    # Bouton retour vers la page principale — navigation via session_state pour rerun immédiat
    if st.sidebar.button("← Retour à l'accueil", key="retour_accueil"):
        # set the page in session state
        st.session_state["page"] = "main"
        # Try to force an immediate rerun in a compatible way
        # 1) preferred: experimental_rerun (older Streamlit)
        if hasattr(st, "experimental_rerun"):
            try:
                st.experimental_rerun()
            except Exception:
                pass
        # 2) try modern query-param setter which also triggers a rerun
        if hasattr(st, "set_query_params"):
            try:
    ...
    DEFAULT_EXCEL = r"DATA\Extraction_gains_2025.xlsx"
    ...
    extra_charts = st.multiselect(
        ...
        # keep the remaining charts selected by default
        default=["Stacked Components", "Pie distribution", "Time series (monthly)"],
    )
    ...
    # (Treemap et Histogram supprimés)
